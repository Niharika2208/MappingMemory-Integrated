...
def file_upload():
    ...
    if raw_data is not None:
        ...
        # st.write("Uploaded file type:", raw_data.type)
        if raw_data.type in ["text/xml", "application/octet-stream", "application/automationml+xml"]:
    ...
    if st.session_state["aml_object"] is not None:
        if st.session_state["type"] == "XML":
            try:
                ...
                # for lib in optimized_dict.get("SystemUnitClassLib", []):
                #    for system_unit in lib.get("SystemUnitClass", []):
                #        system_unit["Additional_Information"] = {}
